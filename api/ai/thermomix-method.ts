...
import { validateThermomixInput } from '../_utils/validation';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!rateCheck.allowed) {
    return res.status(429).json({ 
      ...
      message: 'Please wait a moment before generating more Thermomix methods.',
  ...
  try {
    ...
    const validation = validateThermomixInput(req.body);
    if (!validation.valid) {
      return res.status(400).json({ error: validation.error });
    }
    
    const { recipe: currentMeal } = validation.sanitizedValue;
